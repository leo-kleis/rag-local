...
- **Versionado de Esquema (.lancedb/meta.json)**: Control de versionado SemVer (`SCHEMA_VERSION = "4.0.0"`) gestionado por `services/meta.py`. Detección automática de re-ingesta forzada ante incompatibilidades o actualizaciones del modelo Pydantic `CodeChunk` (vectores de 1024 dimensiones).
...
- **Local Embeddings & Reranker (1024D)**: Inferencia 100% local y offline usando ONNX Runtime GPU (CUDA) con `onnx-community/bge-m3-ONNX` (1024D) y `onnx-community/bge-reranker-v2-m3-ONNX` (FP16) acelerados con `hf-xet` en Hugging Face Xet Storage. Soporta modo Standalone y modo Daemon (precarga permanente en VRAM), compartiendo un límite unificado de reserva del 72% de VRAM (`DAEMON_VRAM_FRACTION=0.72`), auto-reducción de batch size y purga de caché post-lote.
