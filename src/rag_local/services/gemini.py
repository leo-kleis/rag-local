...
def get_embeddings(texts: list[str]) -> list[list[float]] | None:
    """Obtiene embeddings de Gemini con backoff exponencial ante rate limits."""
    if not genai_client:
        raise ValueError(
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if (
                ...
                logger.warning(
                    f"API saturada ({e.code or 'High Demand'}): {e.message}. "
                ...
                logger.error(f"Error de API Gemini en get_embeddings: {e}")
                raise e
        except Exception as e:
            logger.error(f"Error inesperado en get_embeddings: {e}")
            raise e
    return None
