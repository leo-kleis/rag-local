...
def generate_html_graph(lancedb_path: Path, base_output_path: Path) -> None:
    ...
    if not records:
        raise ValueError("No hay relaciones registradas en la base de datos.")
    ...
    for r in records:
        ...
        mermaid_lines.append(
            f'    {m_src}["{esc_src}"] -->|{rel_type}| {m_tgt}["{esc_tgt}"]'
        )
    mermaid_code = "\n".join(mermaid_lines)
    ...
    if (
        not template_3d_path.exists()
        or not template_2d_path.exists()
        or not template_mermaid_path.exists()
    ):
        raise FileNotFoundError(
            f"No se encontraron los archivos de plantilla HTML en: {templates_dir}"
        )

    html_3d = template_3d_path.read_text(encoding="utf-8").replace(
        "__GRAPH_DATA__", graph_data_json
    )

    # Inyectar tanto la versión simplificada como la versión completa
    html_2d = (
        template_2d_path.read_text(encoding="utf-8")
    ...
    html_mermaid = template_mermaid_path.read_text(encoding="utf-8").replace(
        "__MERMAID_CODE__", escaped_mermaid
    )
