...
- **Mantener el Índice Actualizado**: Revisa siempre primero la configuración (herramienta `get_config`) para saber si debes hacer `ingest` o no. Si creas nuevos archivos o aplicas modificaciones estructurales significativas (en Angular, NestJS, Prisma o Python), ejecuta la herramienta `ingest_codebase` para regenerar los hashes de caché e indexar el código de forma incremental.
